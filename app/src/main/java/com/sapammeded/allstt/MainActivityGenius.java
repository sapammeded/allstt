...
public class MainActivityGenius extends MainActivity {
    ...
    public class CentralBridge {
        @JavascriptInterface public void getCentral(String gasUrl, String spreadsheetId, String requestId) {
            try {
                ...
                String url = gasUrl + sep + "action=getCentral&spreadsheetId=" + java.net.URLEncoder.encode(spreadsheetId, "UTF-8") + "&_=" + System.currentTimeMillis();
                finish(requestId, true, unwrapJsonp(request(url, "GET", null)));
            } catch (Exception e) { finish(requestId, false, e.getMessage()); }
        }

        @JavascriptInterface public void postCentral(String gasUrl, String body, String requestId) {
            try {
                URL base = new URL(gasUrl);
                if (!allowedHost(base.getHost())) throw new Exception("Host CENTRAL tidak diizinkan");
                finish(requestId, true, request(gasUrl, "POST", body == null ? "" : body));
            } catch (Exception e) { finish(requestId, false, e.getMessage()); }
        }

        private boolean allowedHost(String host) {
            return "script.google.com".equalsIgnoreCase(host) || "script.googleusercontent.com".equalsIgnoreCase(host);
        }

        private String request(String target, String method, String body) throws Exception {
            String current = target, currentMethod = method, currentBody = body;
            for (int redirects = 0; redirects < 6; redirects++) {
                ...
                c.setRequestProperty("Accept-Language", "id-ID,id;q=0.9,en;q=0.7"); c.setRequestMethod(currentMethod);
                if ("POST".equals(currentMethod)) {
                    c.setDoOutput(true); c.setRequestProperty("Content-Type", "text/plain;charset=utf-8");
                ...
                try (InputStream input = in) { byte[] buf = new byte[8192]; int n; while ((n = input.read(buf)) != -1) out.write(buf, 0, n); }
                String text = out.toString(StandardCharsets.UTF_8.name()); c.disconnect();
        ...
        private String unwrapJsonp(String text) {
            if (text == null) return "null"; String t = text.trim();
            if (t.startsWith("{") || t.startsWith("[")) return t;
            int open = t.indexOf('('), close = t.lastIndexOf(')');
            return open >= 0 && close > open ? t.substring(open + 1, close).trim() : t;
        }
        ...
        private void finish(String requestId, boolean ok, String data) {
            ...
            runOnUiThread(() -> { if (hvssWebView != null) hvssWebView.evaluateJavascript(js, null); });
        }
    }
}
