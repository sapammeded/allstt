...
# Remove legacy identity editor that was creating "Pengaturan Identitas Perusahaan"
# directly on the normal dashboard. Identity editing is now ADMIN-only.
s = re.sub(r'<script[^>]+id=["\']ALLSTT_COMPANY_IDENTITY_V1["\'][^>]*>.*?</script>', '', s, flags=re.S|re.I)
s = re.sub(r'<style[^>]+id=["\']allstt-company-identity-css["\'][^>]*>.*?</style>', '', s, flags=re.S|re.I)
s = re.sub(r'<div[^>]+id=["\']allsttCompanyIdentityPanel["\'][^>]*>.*?</div>', '', s, flags=re.S|re.I)
...
function install(){
  ...
  function openCamera(){reset();cam.value='';cam.setAttribute('accept','image/*');cam.setAttribute('capture','user');cam.click();}
  function openGalleryWithPassword(){
    reset();
    const entered=window.prompt('Masukkan password untuk Upload Foto dari Galeri:');
    if(entered===null) return;
    if(entered!==adminPassword()){window.alert('❌ Password salah. Akses galeri ditolak.');return;}
  ...
  // Do not use DataTransfer/files assignment on Android WebView. Call the baseline processor directly.
  gal.addEventListener('change',async function(e){
    const file=e.target.files&&e.target.files[0];e.target.value='';
    if(!file)return;
    if(!window.__ALLSTT_FACE_GALLERY_AUTH){window.alert('🔒 Password diperlukan.');return;}
    ...
    try{
      ...
      showOverlay('Memproses foto wajah dari galeri...');setOverlayProgress(25);
      await window.__ALLSTT_SAVE_OFFICER_PHOTO(file);setOverlayProgress(100);hideOverlay();
    }catch(err){hideOverlay();window.alert('❌ Foto wajah dari galeri gagal diproses: '+(err&&err.message?err.message:err));}
  },true);
}
...
#allsttCompanyIdentityPanel{display:none!important}
...
function protect(){
  if(unlocked)return;
  ['#companySection','#logoUploadSection','#allsttCompanyIdentityPanel'].forEach(function(id){var e=q(id);if(e)e.style.display='none'});
}
function getAdminPassword(){try{return typeof getLogoUploadPassword==='function'?getLogoUploadPassword():(typeof getGalleryPassword==='function'?getGalleryPassword():'mbahpritampan')}catch(e){return 'mbahpritampan'}}
function unlock(){
  var field=q('#allsttAdminPassword'),pass=field?field.value:'';
  if(pass!==getAdminPassword()){alert('❌ Password admin salah.');return;}
  unlocked=true;window.allsttAdminUnlocked=true;window.allsttGalleryUnlocked=true;document.body.classList.add('allstt-admin-unlocked');
  try{window.logoUnlocked=true}catch(e){}
  var m=q('#allsttAdminGate');if(m)m.style.display='none';if(field)field.value='';
  var sec=q('#companySection');if(sec)sec.style.display='block';var logo=q('#logoUploadSection');if(logo)logo.style.display='block';
  try{renderAreas()}catch(e){}
  alert('✅ Password benar. Mode ADMIN aktif.');
}
function gate(){
  var m=q('#allsttAdminGate');
  if(!m){m=document.createElement('div');m.id='allsttAdminGate';m.innerHTML='<div class="box"><h2 style="margin:0 0 8px;color:#0b3d91"><i class="fas fa-user-shield"></i> ADMIN</h2><div style="color:#64748b">Fitur admin hanya dapat digunakan oleh pengguna yang mengetahui password admin.</div><input id="allsttAdminPassword" type="password" autocomplete="current-password" placeholder="Password admin"><div style="display:flex;gap:8px"><button id="allsttAdminCancel" type="button" class="btn btn-ghost" style="flex:1">BATAL</button><button id="allsttAdminLogin" type="button" class="btn btn-primary" style="flex:1">MASUK</button></div></div>';document.body.appendChild(m);
    q('#allsttAdminCancel').onclick=function(){m.style.display='none'};q('#allsttAdminLogin').onclick=unlock;q('#allsttAdminPassword').onkeydown=function(e){if(e.key==='Enter')unlock()};}
  m.style.display='flex';q('#allsttAdminPassword').value='';q('#allsttAdminPassword').focus();
}
function addAdmin(){
  ...
  var host=qa('.side-nav')[0];if(host)host.appendChild(b);
}
function addPanel(){
  if(q('#allsttAdminPanel'))return;var company=q('#companySection');if(!company)return;
  var panel=document.createElement('div');panel.id='allsttAdminPanel';panel.innerHTML='<div style="font-weight:900;color:#0b3d91"><i class="fas fa-user-shield"></i> MODE ADMIN AKTIF</div><div style="font-size:13px;color:#64748b;margin-top:5px">Identitas perusahaan, logo kiri/kanan, dan multiple upload galeri sekarang dapat digunakan.</div><button id="allsttAdminLock" type="button" class="btn btn-ghost" style="margin-top:10px;width:100%">KUNCI KEMBALI ADMIN</button>';
  company.parentNode.insertBefore(panel,company);
  q('#allsttAdminLock').onclick=function(){unlocked=false;window.allsttAdminUnlocked=false;window.allsttGalleryUnlocked=false;document.body.classList.remove('allstt-admin-unlocked');try{window.logoUnlocked=false}catch(e){}protect();try{renderAreas()}catch(e){}};
}
...
# Patrol multiple-upload controls are ADMIN-only. The face-photo double-tap remains password protected separately.
...
# Remove legacy gallery-password editor from the normal patrol UI.
