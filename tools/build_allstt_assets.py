...
for p in modules:
    ...
    if p.name.lower() == 'stt.html':
        ...
        # Make the Android build use the deployed STTFINO3 CENTRAL.
        text, n_central = re.subn(
            r'const\s+STT_FINDING_CENTRAL_URL\s*=\s*[^;]+;',
            f'const STT_FINDING_CENTRAL_URL = {CENTRAL_URL!r};',
            text, count=1
        )
        if n_central == 0:
            print('WARNING: STT_FINDING_CENTRAL_URL not found')
...
# Unified STTFINO3 Code.gs deployment URL.
device_url = CENTRAL_URL
...
print('STTFINO3 CENTRAL:', CENTRAL_URL)
